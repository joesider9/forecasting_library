...
class MLP():
    ...
    def build_graph(self, x1, y_pred_, learning_rate, units, hold_prob, act_func):
        ...
        with tf.name_scope("train_mlp") as scope:
            if self.probabilistic:
                ...
                cost_mlp = tf.reduce_mean(tf.square(y_pred - y_pred_))
                optimizer_mlp = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
                train_mlp = optimizer_mlp.minimize(cost_mlp)
                accuracy_mlp = tf.reduce_mean(tf.abs(y_pred - y_pred_))
                sse_mlp = tf.reduce_sum(tf.square(y_pred - y_pred_))
                rse_mlp = tf.sqrt(tf.reduce_mean(tf.square(y_pred - y_pred_)))
    ...
    def train(self, max_iterations=10000, learning_rate=5e-4, units=32, hold_prob=1, act_func='elu', gpu_id='/cpu:0'):
        ...
        with graph_mlp.as_default():
            with tf.device(gpu_id):
                x1 = tf.compat.v1.placeholder('float', shape=[None, H_train.shape[1]], name='input_data')
                y_pred_ = tf.compat.v1.placeholder(tf.float32, shape=[None, y_train.shape[1]], name='target_mlp')

            with tf.device(gpu_id):
                train_mlp, cost_mlp, accuracy_mlp, sse_mlp, rse_mlp, weights = self.build_graph(x1, y_pred_,
                                                                                                learning_rate, units,
                                                                                                hold_prob, act_func)
                ...
                for i in tqdm(range(max_iterations)):
                    if i % 500 == 0:
                        ...
                        acc_new_t, mse_new_t, sse_new_t, rse_new_t = sess.run(
                            [accuracy_mlp, cost_mlp, sse_mlp, rse_mlp],
