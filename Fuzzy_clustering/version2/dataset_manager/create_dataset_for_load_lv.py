...
class dataset_creator_LV():
    ...
    def make_dataset_lv(self):
        ...
        for project in self.projects:
            ...
            if self.isfortest:
                dataset_X, dataset_y, X_3d = self.create_dataset(X[project['_id']], data_path, start_index=300,
                                                                 test=self.isfortest)
            ...
            try:
                ...
                var_3d = np.hstack((self.data.loc[date, 'RESForecast'] + self.data.loc[date, 'LoadForecast'],
                                    nwps_lstm[['cloud', 'wind', 'direction', 'Temp_max', 'Temp_min',
                                               'Temp_athens', 'Temp_thessaloniki', 'Temp_ioannina', 'Temp_larissa',
                                               'Temp_patra']].loc[date].values,
                ...
                for d in date_pred:
                    ...
                    v = np.hstack(
                        (self.data.loc[d, 'RESForecast'] + self.data.loc[d, 'LoadForecast'],
                ...
                for d in date_inp1:
                    ...
                    v = np.hstack(
                        (self.data.loc[d, 'APE_net'] + self.data.loc[d, 'SCADA'],
                         nwps_lstm[
                             ['cloud', 'wind', 'direction', 'Temp_max', 'Temp_min', 'Temp_athens', 'Temp_thessaloniki',
                              'Temp_ioannina', 'Temp_larissa', 'Temp_patra']].loc[d].values,
                         self.data[['hour', 'month', 'sp_index', 'dayweek']].loc[d].values,
                         np.power(self.data['month'].loc[d] * temp_max / 12, 3),
                         np.power(self.data['sp_index'].loc[d] * temp_max / 100, 3)))
                    var_3d = np.vstack((var_3d, v))
    ...
    def make_dataset_lv_online(self):
        ...
        start_date = X[project['_id']].index[-1] + pd.DateOffset(hours=1)
        start_date = pd.to_datetime(start_date.strftime('%d%m%y'), format='%d%m%y')
        end_date = pd.to_datetime(self.dates[0].strftime('%d%m%y'), format='%d%m%y')
        dates = pd.date_range(start_date, end_date)
        data_dates = pd.to_datetime(np.unique(self.data.index.strftime('%d%m%y')), format='%d%m%y')
        dates = [d for d in dates if d in data_dates]
        dates = pd.DatetimeIndex(dates)
        if dates.shape[0] > 0:
            nwp = self.stack_daily_nwps_online(self.dates[0], self.data, lats, longs, self.path_nwp, self.nwp_model,
                                               self.projects, self.variables,
                                               self.compress)
            # nwp_daily = Parallel(n_jobs=self.njobs)(
            #     delayed(self.stack_daily_nwps_online)(t, self.data, lats, longs, self.path_nwp, self.nwp_model, self.projects,
            #                                    self.variables,
            #                                    self.compress) for t in dates)

            if nwp[0][project['_id']].shape[0] != 0:
                X[project['_id']] = pd.concat([X[project['_id']], nwp[0][project['_id']]])

            else:
                raise ImportError('Cannot find NWP')
            X[project['_id']].to_csv(os.path.join(project['static_data']['path_data'], file_nwp))
        data_path = project['static_data']['path_data']

        dataset_X, X_3d = self.create_dataset_online(X[project['_id']], data_path, start_index=300, test=self.isfortest)
    ...
    def create_dataset_online(self, nwps, data_path, start_index=9001, test=False):
        ...
        for d in date_inp1:
            ...
            v = np.hstack(
                (self.data.loc[d, 'APE_net'] + self.data.loc[d, 'SCADA'],
                 nwps_lstm[['cloud', 'wind', 'direction', 'Temp_max', 'Temp_min', 'Temp_athens', 'Temp_thessaloniki',
                            'Temp_ioannina', 'Temp_larissa', 'Temp_patra']].loc[d].values,
                 self.data[['hour', 'month', 'sp_index', 'dayweek']].loc[d].values,
                 np.power(self.data['month'].loc[d] * temp_max / 12, 3),
                 np.power(self.data['sp_index'].loc[d] * temp_max / 100, 3)))
