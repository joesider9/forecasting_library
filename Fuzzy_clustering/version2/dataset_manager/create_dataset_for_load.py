...
class dataset_creator_scada():
    ...
    def make_dataset_scada(self):
        ...
        for project in self.projects:
            ...
            if self.isfortest:
                dataset_X, dataset_y, X_3d = self.create_dataset(X[project['_id']], data_path, start_index=9000,
                                                                 test=self.isfortest)
                ...
                dataset_X, dataset_y, X_3d = self.create_dataset(X[project['_id']], data_path, test=self.isfortest)
                if dataset_y.isna().any().values[0]:
                    dataset_X = dataset_X.drop(dataset_y.index[np.where(dataset_y.isna())[0]])
    ...
    def create_dataset(self, nwps, data_path, start_index=9001, test=False):
        ...
        for date in self.data.index[start_index:]:
            ...
            try:
                ...
                temp_max = nwps[['Temp_max']].loc[date].values
                var_3d = np.hstack((np.array([0]), self.data.loc[date, 'APE_net'],
                                    self.data.loc[date, 'APE_net'] + self.data.loc[date, 'SCADA'],
                ...
                for d in date_inp1:
                    ...
                    v = np.hstack(
                        (self.data.loc[d, 'SCADA'], self.data.loc[d, 'APE_net'],
