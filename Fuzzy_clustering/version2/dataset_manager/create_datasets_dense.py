...
class dataset_creator_dense():
                    ...
                    for date in pdates:
                        try:
                            ...
                            if self.check_empty_nwp(nwp, nwp_next, nwp_prev, variables):
                                y[project['_id']] = pd.concat([y[project['_id']],
                                                               pd.DataFrame(data.loc[date, project['_id']],
                                                                            columns=['target'], index=[date])])
                                inp, inp_cnn = self.create_sample(date, nwp, nwp_prev, nwp_next, lats[project['_id']],
    ...
    def make_dataset_res(self):
        ...
        nwp = self.stack_daily_nwps(self.dates[0], self.data, lats, longs, self.path_nwp, self.nwp_model, self.projects,
                                    self.variables,
                                    self.compress)
        ...
        if self.isfortest:
            for project in self.projects:
                ...
                joblib.dump(X_3d[project['_id']],
                            os.path.join(project['static_data']['path_data'], 'dataset_cnn_test.pickle'))
            ...
            for project in self.projects:
                ...
                joblib.dump(X_3d[project['_id']],
                            os.path.join(project['static_data']['path_data'], 'dataset_cnn.pickle'))
                self.logger.info('Datasets saved for project %s', project['_id'])
