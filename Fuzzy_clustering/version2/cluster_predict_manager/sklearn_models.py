    ...
    def predict(self, X):
        if self.istrained == True and hasattr(self, 'models'):
            if self.method in self.models.keys():
                ...
                raise ImportError('SKlearn Manager has not attribute %s for cluster %s of project', self.method,
                                  self.cluster.cluster_name, self.static_data['_id'])
        else:
            raise ImportError('SKlearn Manager has not attribute models for cluster %s of project',
                              self.cluster.cluster_name, self.static_data['_id'])
