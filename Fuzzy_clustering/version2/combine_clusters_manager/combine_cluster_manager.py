...
from Fuzzy_clustering.version2.combine_clusters_manager.data_sampler import DataSampler
        ...
        if X_test.shape[0] > 0 and len(self.methods) > 1 and self.istrained == False:
            ...
            for combine_method in self.combine_methods:
                if combine_method == 'rls':
                    ...
                    mlp_model = sklearn_model(self.static_data, self.model_dir, self.rated, 'mlp', self.n_jobs,
                                              is_combine=True, path_group=self.static_data['path_group'])
