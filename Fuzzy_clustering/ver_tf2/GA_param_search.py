...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__, njobs=1,
             name_values=(), X=(), y=(),
             scorer=None, cvs=[], iid=None, verbose1=0,
             error_score=None, fit_params=None,
             score_cache=None, estimator=None):
    ...
    fitnesses = Parallel(n_jobs=njobs, max_nbytes=None)(delayed(toolbox.evaluate)(np.array(individual).ravel()
    ...
    #                             args=(np.array(individual), name_values, X, y, scorer, cvs, iid, fit_params,
    #                                   verbose1, error_score, score_cache, estimator))
    #            for individual in
    #            invalid_ind]
...
def _evalFunction(individual, name_values, X, y, scorer, cvs, iid, fit_params,
    ...
    if paramkey in score_cache:
        ...
        score = fit_and_score(estimator, cvs, parameters)
...
class EvolutionaryAlgorithmSearchCV(BaseSearchCV):
    ...
    def __init__(self, estimator, params, scoring=None, cv=4,
                 ...
                 n_jobs=1, iid=True, error_score='raise', init_params=[],
                 fit_params={}):
    ...
    def _fit(self, X, y, cvs, parameter_dict):
        ...
        pop, logbook = eaSimple(pop, toolbox, cxpb=0.5, mutpb=0.5,
                                ...
                                halloffame=hof, verbose=self.verbose, njobs=self.n_jobs,
                                name_values=name_values, X=X, y=y,
                                scorer=self.scorer_, cvs=cvs, iid=self.iid, verbose1=self.verbose,
                                error_score=self.error_score, fit_params=self.fit_params,
                                score_cache=self.score_cache, estimator=self.estimator)
