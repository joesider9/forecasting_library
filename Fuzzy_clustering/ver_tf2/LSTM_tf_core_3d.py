...
class LSTM_3d():
    ...
    def build_graph(self, x1, y_pred_, learning_rate, units, hold_prob):
        ...
        with tf.name_scope("train_lstm") as scope:
            if self.probabilistic:
                ...
                cost_lstm = tf.reduce_mean(tf.square(y_pred - y_pred_))
                optimizer_lstm = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
                train_lstm = optimizer_lstm.minimize(cost_lstm)
                accuracy_lstm = tf.reduce_mean(tf.abs(y_pred - y_pred_))
                sse_lstm = tf.reduce_sum(tf.square(y_pred - y_pred_))
                rse_lstm = tf.sqrt(tf.reduce_mean(tf.square(y_pred - y_pred_)))
    ...
    def train(self, max_iterations=10000, learning_rate=5e-4, units=32, hold_prob=1, gpu_id='/cpu:0'):
