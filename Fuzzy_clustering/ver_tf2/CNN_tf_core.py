...
class CNN():
    ...
    def create_inputs(self, X_train, X_val, X_test):
        ...
        for i in range(self.depth):
            ...
            H_train.append(np.nan_to_num(np.transpose(self.rbf_map(X_train, self.num_centr, centroids, Radius), [1, 2, 0])))
            H_val.append(np.nan_to_num(np.transpose(self.rbf_map(X_val, self.num_centr, centroids, Radius), [1, 2, 0])))
            H_test.append(np.nan_to_num(np.transpose(self.rbf_map(X_test, self.num_centr, centroids, Radius), [1, 2, 0])))
    ...
    def build_graph(self, x1, y_pred_, learning_rate, kernels, h_size, hold_prob, filters):
        ...
        with tf.name_scope("train_cnn") as scope:
            cost_cnn = tf.reduce_mean(tf.square(y_pred - y_pred_))
            optimizer_cnn = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
            train_cnn = optimizer_cnn.minimize(cost_cnn)
            accuracy_cnn = tf.reduce_mean(tf.abs(y_pred - y_pred_))
            sse_cnn = tf.reduce_sum(tf.square(y_pred - y_pred_))
            rse_cnn = tf.sqrt(tf.reduce_mean(tf.square(y_pred - y_pred_)))
    ...
    def train_cnn(self, max_iterations=10000, learning_rate=5e-4, kernels=[2,8], h_size=[248,52], gpu_id='/cpu:0', filters=24):
                ...
                for i in tqdm(range(max_iterations)):
                    if i % 500 == 0:
                        ...
                        acc_new_v, mse_new_v, sse_new_v, rse_new_v, weights_cnn = sess.run([accuracy_cnn, cost_cnn, sse_cnn, rse_cnn, weights],
                                                    feed_dict={x1: H_val, y_pred_: y_val, hold_prob:1})
                        acc_new_t, mse_new_t, sse_new_t, rse_new_t = sess.run([accuracy_cnn, cost_cnn, sse_cnn, rse_cnn],
                                                    feed_dict={x1: H_test, y_pred_: y_test, hold_prob:1})
