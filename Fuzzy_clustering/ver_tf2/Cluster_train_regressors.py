...
class cluster_train(object):
    def __init__(self, static_data, clust, x_scaler):
        ...
        logger = logging.getLogger('clusters_train_procedure_' + self.cluster_name )
    ...
    def find_features(self, cvs, method, njobs):
        self.logger.info('Start feature selection %s', self.cluster_name)
        if method=='boruta':
    ...
    def fit(self):
        ...
        if not self.istrained:
            ...
            self.logger.info('Cluster of %s loaded successfully', self.cluster_name)
...
if __name__=='__main__':
    ...
    cluster_models = cluster_train(static_data, 'rule.8', model.sc)

    model = cluster_models.fit()
