...
def rbf_optim_single(net, X, y, X_val, y_val, X_test, y_test, width, keep, rated):
    ...
    return (metrics, {'centroids': net.model['centroids'], 'Radius': net.model['Radius'], 'W': net.model['W'], 'acc':  0.6 *mae_test + 0.4 * mae_val})
...
class rbf_ols():
    ...
    def train_ols(self, I, O, X_val, y_val, X_test, y_test, rated, gw=1.0, keep=3, verbose=False):
        ...
        if not flag:
            ...
            centers = I[used]
            ibias = k
        while remains<keep and P.shape[1] > 0 and used.shape[0]<=120:
            if verbose:
...
class rbf_ols_module(object):
    ...
    def optimize_rbf(self, cvs):
        ...
        if X.shape[0]>3000:
            ind = np.random.randint(low=0, high=X.shape[0], size=3000)
            X = X[ind]
            y = y[ind]
        if self.GA==True:
    ...
    def optimize_rbf_TL(self, cvs, models):
        X = cvs[0][0]
        y = cvs[0][1].reshape(-1, 1)
        X_val = cvs[0][2]
        y_val = cvs[0][3].reshape(-1, 1)
    ...
    def optimize_common_width(self, X, y, X_val, y_val, X_test, y_test, rated):
        ...
        # result= [pool.apply_async(rbf_optim_single, args=(net, X, y, X_val, y_val, X_test, y_test, w,3, rated)) for w in widths]
        #
        ...
        results = Parallel(n_jobs=self.njobs)(
            delayed(rbf_optim_single)(net, X, y, X_val, y_val, X_test, y_test, w, 5, rated) for
            w in widths)
    ...
    def optimize_common_width_TL(self, X, y, X_val, y_val, X_test, y_test, rated, models):
        ...
        for k in range(3):
            net = rbf_ols()
            if k>=len(models['models']):
                k = len(models['models'])-1
            results = rbf_optim_single(net, X, y, X_val, y_val, X_test, y_test, models['models'][k]['Radius'], 5, rated)

            self.models.append(results[1])
            if len(self.models)==3:
                break
    ...
    def optimize_with_deep_TL(self, X, y, X_val, y_val, X_test, y_test, rated, models):
        ...
        for k in range(3):
            ...
            if k>=len(models['models']):
                k = len(models['models'])-1
            err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=models['models'][k]['Radius'], keep=5, verbose=True)
            pred_test = net.predict(X_test)
            pred_val = net.predict(X_val)
            sse_test, rms_test, mae_test, mse_test = net.compute_metrics(pred_test, y_test, rated)
            sse_val, rms_val, mae_val, mse_val = net.compute_metrics(pred_val, y_val, rated)
            self.models.append({'centroids': net.model['centroids'], 'Radius': net.model['Radius'], 'W': net.model['W'], 'acc':  float(0.6 *mae_test + 0.4 * mae_val)})
            if len(self.models)==3:
                break
    ...
    def optimize_with_deep(self, X, y, X_val, y_val, X_test, y_test, rated):
        ...
        def rbf_optim(individual):
            net = rbf_ols()
            err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=individual, keep=5, verbose=True)
            pred_test = net.predict(X_test)
            pred_val = net.predict(X_val)
            sse_test, rms_test, mae_test, mse_test = net.compute_metrics(pred_test, y_test, rated)
            sse_val, rms_val, mae_val, mse_val = net.compute_metrics(pred_val, y_val, rated)
        ...
        net = rbf_ols()
        population, logbook = optimize(toolbox.population(), toolbox, 50, 50, net, X, y, X_val, y_val, X_test, y_test, rated, self.njobs,  cxpb=0.6, mutpb=0.4,
        ...
        for k in range(3):
            ...
            inds = np.array([i for i in range(len(population)) if population[i].fitness.getValues()[0] == population[best_ind].fitness.getValues()[0]])


            net = rbf_ols()
            err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=np.array(population[best_ind]), keep=5, verbose=True)
            pred_test = net.predict(X_test)
            pred_val = net.predict(X_val)
            sse_test, rms_test, mae_test, mse_test = net.compute_metrics(pred_test, y_test, rated)
            sse_val, rms_val, mae_val, mse_val = net.compute_metrics(pred_val, y_val, rated)
            logger.info('Best model centers %s acc %s', net.model['centroids'].shape[0], float(0.6 *mae_test + 0.4 * mae_val))
            self.models.append({'centroids': net.model['centroids'], 'Radius': net.model['Radius'], 'W': net.model['W'], 'acc':  float(0.6 *mae_test + 0.4 * mae_val)})
...
def rbf_optim(individual,net, X, y, X_val, y_val, X_test, y_test, rated):
        ...
        err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=np.array(individual), keep=5, verbose=False)
        pred_test = net.predict(X_test)
        pred_val = net.predict(X_val)
        sse_test, rms_test, mae_test, mse_test = net.compute_metrics(pred_test, y_test, rated)
        sse_val, rms_val, mae_val, mse_val = net.compute_metrics(pred_val, y_val, rated)
...
def optimize(population, toolbox, mu, lambda_, net, X, y, X_val, y_val, X_test, y_test, rated, njobs, cxpb, mutpb, ngen,
    ...
    #     fitnesses.append(m)
    # pool=mp.Pool(njobs)
    # results=[pool.apply_async(rbf_optim, args=(np.array(individual).ravel(), net, X, y, X_val, y_val, X_test, y_test, rated)) for individual in invalid_ind]
