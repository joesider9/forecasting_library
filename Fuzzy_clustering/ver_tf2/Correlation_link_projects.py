...
import joblib, os, difflib, logging, copy
...
class ProjectLinker():
    ...
    def linked_with_regression(self):
        ...
        for project1 in self.projects:
            ...
            model1 = sklearn_model(path_regressor, rated, 'svm', project1['static_data']['njobs'])
            ...
            for project2 in self.projects:
                ...
                if project1['_id'] != project2['_id']:
                    ...
                    for i in range(3):
                        ...
                        cvs.append([X_train, y_train, X_val, y_val, X_test1, y_test1])
                    model2 = sklearn_model_tl(path_regressor, rated, 'svm', project2['static_data']['njobs'])
                    _ = model2.train(cvs, model1.best_params)
                    self.corr_reg.loc[project1['_id'], project2['_id']] = model2.acc_test
                    self.logger.info('Error computed for svm regressor traine with transfer learning from project %s to project %s', project1['_id'], project2['_id'])
