...
def base_minimize(func, dimensions, base_estimator,
    ...
    for n in range(n_calls):
        next_x = optimizer.ask(n_points=20)
        next_y = Parallel(n_jobs=n_jobs)(delayed(func)(v) for v in next_x)
    ...
    return result
# ext_x = [optimizer.ask() for _ in range(40)]
#         pool = mp.Pool(n_jobs)
#         res = [pool.apply_async(func(x)) for x in next_x]
#         next_y = [p.get() for p in res]
#         pool.close()
#         pool.terminate()
#         pool.join()