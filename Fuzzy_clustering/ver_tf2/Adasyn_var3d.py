...
class ADASYN():
    ...
    def fit_resample(self, X, X_3d, y, y_org):
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            ...
            diffs_3d = X_class_3d[nns[rows, cols]] - X_class_3d[rows]
            ...
            if sparse.issparse(X):
                ...
                X_new_3d = X_class_3d[rows] + steps[:,:, np.newaxis, np.newaxis].multiply(diffs)
            else:
                X_new[:, self.variables] = X_class[rows][:, self.variables] + steps * diffs
                X_new_3d = X_class_3d[rows] + steps[:,:, np.newaxis, np.newaxis] * diffs_3d
