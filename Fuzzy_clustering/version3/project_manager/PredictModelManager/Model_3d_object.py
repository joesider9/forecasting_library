...
import os
import numpy as np
from Fuzzy_clustering.version3.project_manager.PredictModelManager.CNN_predict_3d import CNN_3d_predict
from Fuzzy_clustering.version3.project_manager.PredictModelManager.CNN_predict import CNN_predict
from Fuzzy_clustering.version3.project_manager.PredictModelManager.LSTM_predict_3d import LSTM_3d_predict
from Fuzzy_clustering.version3.project_manager.PredictModelManager.MLP_predict_3d import MLP_predict
...
class model3d_object():
    def __init__(self, static_data, cluster, method):
        ...
        if not self.istrained:
            raise ImportError('Cannot find model %s for cluster %s', self.method, self.cluster_name)
    ...
    def predict(self, X, rbf_models = None):
        if self.istrained:
            ...
            raise ImportError('Model %s is not trained for cluster %s of project', self.method, self.cluster.cluster_name, self.static_data['_id'])
    ...
    def predict_mlp(self, X):
        model_predict = MLP_predict(self.static_data, self.trial, self.probabilistic)
        if self.istrained:
            pred = model_predict.predict(X)
            pred[np.where(pred < 0)] = 0
            pred[np.where(pred > 1)] = 1
