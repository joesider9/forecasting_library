import joblib
from Fuzzy_clustering.version3.LSTM_Manager.LSTM_tf_core_3d import LSTM_3d
import pika, uuid, time, json, os
import numpy as np
from rabbitmq_rpc.server import RPCServer
from Fuzzy_clustering.version3.LSTM_Manager.Cluster_object import cluster_object

RABBIT_MQ_HOST = os.getenv('RABBIT_MQ_HOST')
RABBIT_MQ_PASS = os.getenv('RABBIT_MQ_PASS')
RABBIT_MQ_PORT = int(os.getenv('RABBIT_MQ_PORT'))

server = RPCServer(queue_name='LSTMmanager', host=RABBIT_MQ_HOST, port=RABBIT_MQ_PORT, threaded=False)
...
def deep_manager(static_data):
    ...
    model_3d = lstm_manager(static_data, cluster, model_method, params=params)
