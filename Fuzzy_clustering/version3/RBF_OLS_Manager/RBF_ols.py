...
class rbf_ols_module(object):
    ...
    def optimize_common_width(self, X, y, X_val, y_val, X_test, y_test, rated):
        ...
        pool=mp.Pool(njobs)
        result= [pool.apply_async(rbf_optim_single, args=(net, X, y, X_val, y_val, X_test, y_test, w,4, rated)) for w in widths]
    ...
    def optimize_with_deep_TL(self, X, y, X_val, y_val, X_test, y_test, rated, models):
        ...
        err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=models['models']['Radius'], keep=4, verbose=True)
        ...
        sse_test, rms_test, mae_test, mse_test = net.compute_metrics(pred_test, y_test, rated)
        sse_val, rms_val, mae_val, mse_val = net.compute_metrics(pred_val, y_val, rated)
        self.models = ({'centroids': net.model['centroids'], 'Radius': net.model['Radius'], 'W': net.model['W'], 'acc':  float(0.6 *mae_test + 0.4 * mae_val)})
    ...
    def optimize_with_deep(self, X, y, X_val, y_val, X_test, y_test, rated):
        ...
        net = rbf_ols()
        population, logbook = optimize(toolbox.population(), toolbox, 40, 60, net, X, y, X_val, y_val, X_test, y_test, rated, self.njobs,self.path_group,  cxpb=0.6, mutpb=0.4,
        ...
        err_train = net.train_ols(X, y, X_val, y_val, X_test, y_test, rated, gw=np.array(population[best_ind]), keep=4, verbose=True)
...
def optimize(population, toolbox, mu, lambda_, net, X, y, X_val, y_val, X_test, y_test, rated, njobs, path_group, cxpb, mutpb, ngen,
    ...
    with elapsed_timer() as eval_elapsed:
        for gen in range(1, ngen + 1):
            ...
            results = [pool.apply_async(rbf_optim,
                                        args=(np.array(individual).ravel(), net, X, y, X_val, y_val, X_test, y_test, rated))
                       for individual in invalid_ind]
            fitnesses = [p.get() for p in results]
            pool.close()
            pool.terminate()
