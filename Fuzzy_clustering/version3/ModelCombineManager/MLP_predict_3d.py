    ...
    def build_graph(self, x1, best_weights, units, hold_prob, act_func):
        ...
        with tf.name_scope("build_mlp") as scope:
            ...
            if self.probabilistic:
                ...
                y_pred, W, b = self.normal_full_layer(full_out_dropout, best_weights['build_mlp/Variable:0'], best_weights['build_mlp/Variable_1:0'] )
        ...
        with graph_mlp.as_default():
            ...
            with tf.device("/cpu:0"):
                if self.trial == 0:
                    ...
                    if self.probabilistic:
                        y_pred_, weights, mlp_1, full_layer_one, outputs = self.build_graph(x1, best_weights, units, 1, act_func)
                    else:
                        y_pred_, weights, mlp_1, full_layer_one = self.build_graph(x1, best_weights, units, 1, act_func)

                elif self.trial == 2:
                    if self.probabilistic:
                        y_pred_, weights, mlp_1, full_layer_two, full_layer_three, outputs = self.build_graph(x1, best_weights, units, 1, act_func)
                    else:
                        y_pred_, weights, mlp_1, full_layer_two, full_layer_three = self.build_graph(x1, best_weights, units, 1, act_func)
                ...
                if self.probabilistic:
                    ...
                    for i, q in enumerate(quantiles):
                        outputs[i].set_weights(
                                [best_weights['build_mlp/{}_q{}/kernel:0'.format(i, int(q * 100))], best_weights['build_mlp/{}_q{}/bias:0'.format(i, int(q * 100))]])
                else:
                    mlp_1.set_weights([best_weights['build_mlp/dense1/kernel:0'], best_weights['build_mlp/dense1/recurrent_kernel:0'], best_weights['build_mlp/dense1/bias:0']])
