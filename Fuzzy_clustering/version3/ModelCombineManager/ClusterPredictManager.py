...
from Fuzzy_clustering.version3.ModelCombineManager.FS_object import FeatSelobject
from Fuzzy_clustering.version3.ModelCombineManager.Model_3d_object import model3d_object
from Fuzzy_clustering.version3.ModelCombineManager.RBF_CNN_model import RBF_CNN_model
from Fuzzy_clustering.version3.ModelCombineManager.SKlearn_object import SKLearn_object
...
class ClusterPredict():
    ...
    def pred_model(self, X, method, X_cnn=np.array([]), X_lstm=np.array([])):
        if (method == 'RBF_ALL'):
            ...
            pred[np.where(pred < 0)] = 0
        elif method in {'SVM', 'NUSVM', 'MLP', 'RF', 'XGB'}:
        ...
        for method in self.methods:
            if X.shape[0] > 0:
                ...
                if method == 'RBF_ALL_CNN':
                    predictions['RBF_OLS'] = 20 * pred[:, 0].reshape(-1, 1)
                    predictions['GA_RBF_OLS'] = 20 * pred[:, 1].reshape(-1, 1)
                    predictions['RBFNN'] = 20 * pred[:, 2].reshape(-1, 1)
                    predictions['RBF-CNN'] = 20 * pred[:, 3].reshape(-1, 1)
                elif method == 'RBF_ALL':
                    predictions['RBF_OLS'] = 20 * pred[:, 0].reshape(-1, 1)
                    predictions['GA_RBF_OLS'] = 20 * pred[:, 1].reshape(-1, 1)
                    predictions['RBFNN'] = 20 * pred[:, 2].reshape(-1, 1)
