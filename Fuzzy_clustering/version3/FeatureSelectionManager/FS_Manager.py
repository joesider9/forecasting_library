...
from Fuzzy_clustering.version3.FeatureSelectionManager.Cluster_object import cluster_object

RABBIT_MQ_HOST = os.getenv('RABBIT_MQ_HOST')
RABBIT_MQ_PASS = os.getenv('RABBIT_MQ_PASS')
RABBIT_MQ_PORT = int(os.getenv('RABBIT_MQ_PORT'))

server = RPCServer(queue_name='FeatSelmanager', host=RABBIT_MQ_HOST, port=RABBIT_MQ_PORT, threaded=False)
...
class FeatSelManager(object):
    ...
    def fit(self):
        cvs = self.load_data()
        if self.method=='boruta':
            from Fuzzy_clustering.version3.FeatureSelectionManager.Feature_selection_boruta import FS
            fs = FS(self.static_data, self.cluster_dir, self.njobs, path_group=self.static_data['path_group'])
        elif self.method=='linearsearch':
            from Fuzzy_clustering.version3.FeatureSelectionManager.Feature_selection_linearsearch import FS
            fs = FS(self.static_data, self.cluster_dir, self.njobs, self.inner_jobs,
                    path_group=self.static_data['path_group'])
        else:
            from Fuzzy_clustering.version3.FeatureSelectionManager.Feature_selection_permutation import FS
            fs = FS(self.static_data, self.cluster_dir, self.njobs, self.inner_jobs, path_group=self.static_data['path_group'])
    ...
    def fit_TL(self):
        ...
        if self.method=='boruta':
            from .Feature_selection_boruta import FS
            fs_trained = FS(static_data_tl, cluster_dir_tl, self.njobs, path_group=self.static_data['path_group'])
        elif self.method=='linearsearch':
            from .Feature_selection_linearsearch import FS
            fs_trained = FS(static_data_tl, cluster_dir_tl, self.njobs, self.inner_jobs,
                    path_group=self.static_data['path_group'])
        else:
            from .Feature_selection_permutation import FS
            fs_trained = FS(static_data_tl, cluster_dir_tl, self.njobs, self.inner_jobs, path_group=self.static_data['path_group'])
...
@server.consumer()
def FeatSelmanager(static_data):
    ...
    if FS_model.istrained == False:
        fs_response = {'result': FS_model.fit(), 'cluster_name': cluster.cluster_name, 'project': static_data['_id']}
    else:
        fs_response = {'result': 'Done', 'cluster_name': cluster.cluster_name, 'project': static_data['_id']}
