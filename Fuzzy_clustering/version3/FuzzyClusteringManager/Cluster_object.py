...
import numpy as np
import pandas as pd
from .utils_for_forecast import split_continuous
...
class cluster_object():
    ...
    def split_dataset(self, X, y, act):
        ...
        cvs = []
        for _ in range(3):
            X_train, X_test, y_train, y_test = split_continuous(X, y, test_size=0.15, random_state=np.random.randint(100))
    ...
    def save_cluster_data(self, X1, y1, X_cnn1, X_lstm1, activations=None, split_test=None):
        ...
        if not os.path.exists(os.path.join(self.data_dir, 'dataset_X.csv')) or self.static_data['recreate_datasets']:
            ...
            if not split_test is None:
                ...
                X = X.iloc[indices_train]
                y = y.iloc[indices_train]
                act = act.iloc[indices_train]

                if len(X_cnn.shape) > 1:
                    X_cnn_test = X_cnn[indices_test]
                    X_cnn = X_cnn[indices_train]
                else:
                    X_cnn_test = np.array([])

                if len(X_lstm.shape) > 1:
                    X_lstm_test = X_lstm[indices_test]
                    X_lstm = X_lstm[indices_train]
                else:
                    X_lstm_test = np.array([])
            else:
                X_test = pd.DataFrame([])
            ...
            if len(X_cnn.shape) > 1:
                ...
                X_cnn_tr, X_cnn_ts, y_cnn_tr, y_cnn_ts = split_continuous(X_cnn, y.values, test_size=0.15, random_state=42)
                X_cnn_tr, X_cnn_val, y_cnn_tr, y_cnn_val = train_test_split(X_cnn_tr, y_cnn_tr, test_size=0.15, random_state=42)
            ...
            if len(X_lstm.shape) > 1:
                ...
                X_lstm_tr, X_lstm_ts, y_lstm_tr, y_lstm_ts = split_continuous(X_lstm, y.values, test_size=0.15, random_state=42)
