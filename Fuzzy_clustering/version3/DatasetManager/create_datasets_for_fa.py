    ...
    def create_dataset(self, nwps, start_index=753):
        ...
        for date in self.data.index[start_index:]:
            ...
            try:
                ...
                var_imp = np.hstack((nwps[['flux','wind', 'cloud', 'direction', 'Temp_max', 'Temp_min']].loc[date].values,
                                           self.data[['month', 'sp_index', 'dayweek']].loc[date].values,
                                     temp,
                                     np.power(self.data['month'].loc[date]*temp/12, 3), np.power(self.data['sp_index'].loc[date]*temp/100, 3)))

                col = ['flux','wind', 'cloud', 'direction', 'Temp_max', 'Temp_min','month', 'sp_index', 'dayweek',  'Temp', 'Temp_month', 'Temp_sp_days']
                ...
                temp = nwps[['Temp']].loc[date].values
                var_3d = np.hstack((np.array([0]),nwps[['flux','wind', 'cloud', 'direction', 'Temp_max', 'Temp_min']].loc[date].values,
                                           self.data[['month', 'sp_index', 'dayweek']].loc[date].values,
                                     temp,
                ...
                for d in date_inp1:
                    ...
                    v = np.hstack(
                        (self.data.loc[d, 'Athens_24'], nwps[['flux','wind', 'cloud', 'direction', 'Temp_max', 'Temp_min']].loc[d].values,
...
class dataset_creator_xmachina():
    ...
    def make_dataset_xmachina_dayahead(self):
        ...
        for project in self.projects:
            data_path = project['static_data']['path_data']
            if self.isfortest:
                dataset_X, dataset_y, X_3d = self.create_dataset_dayahead(data_path, start_index=372, test = self.isfortest)
                ...
                dataset_X, dataset_y, X_3d = self.create_dataset_dayahead(data_path, start_index=730, test = self.isfortest)
    ...
    def create_dataset_curr(self, data_path, start_index=753, test=False):
        ...
        col = ['Ath6_0', 'temp_min', 'temp_mean', 'rh', 'precip', 'hdd_h', 'hdd_h2', 'month', 'sp_index', 'dayweek', 'temp_max',
        ...
        for date in self.data.index[start_index:]:
            ...
            col = ['Ath6_0', 'temp_min', 'temp_mean', 'rh', 'precip', 'hdd_h', 'hdd_h2', 'month', 'sp_index', 'dayweek', 'temp_max', 'Temp_month', 'Temp_sp_days']
            ...
            var_3d = np.hstack((np.array([0]), self.data.loc[date, 'Athens_6'], self.data[['temp_min', 'temp_mean',
                                'rh', 'hdd_h', 'hdd_h2']].loc[date].values,
                                 temp,
    ...
    def create_dataset_dayahead(self, data_path, start_index=753, test=False):
        ...
        col = ['temp_min', 'temp_mean', 'rh', 'precip', 'hdd_h', 'hdd_h2', 'month', 'sp_index', 'dayweek', 'temp_max',
               'Temp_month', 'Temp_sp_days']
        col += ['Ath6_0', 'temp_min_curr', 'temp_mean_curr', 'rh_curr', 'precip_curr', 'hdd_h_curr', 'hdd_h2_curr', 'month_curr',
        ...
        for date in self.data.index[start_index:]:
            ...
            col = ['temp_min', 'temp_mean', 'rh', 'precip', 'hdd_h', 'hdd_h2', 'month', 'sp_index', 'dayweek', 'temp_max', 'Temp_month', 'Temp_sp_days']
            ...
            col += ['Ath6_0', 'temp_min_curr', 'temp_mean_curr', 'rh_curr', 'precip_curr', 'hdd_h_curr', 'hdd_h2_curr', 'month_curr', 'sp_index_curr', 'dayweek_curr',
                   'temp_max_curr', 'Temp_month_curr', 'Temp_sp_days_curr']
            ...
            var_3d = np.hstack((np.array([0]), np.array([0]), self.data[['pred_temp_min', 'pred_temp_mean',
                                'pred_rh', 'pred_hdd_h', 'pred_hdd_h2']].loc[date].values,
                                 temp,
                                 self.data.loc[date_inp1[0], 'Αριθμός Πελατών από 1/1/2017'],
                                 np.power(self.data['month'].loc[date] * temp / 12, 3),
                                 np.power(self.data['sp_index'].loc[date] * temp / 100, 3)))
